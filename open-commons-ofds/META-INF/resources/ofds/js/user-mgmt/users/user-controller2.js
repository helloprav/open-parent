(function() {
    ...
    angular.module('routerApp').controller(
		...
		// Start: Server Side Pagination
	    $scope.sortColSelected = ''; // Default sorting applied on column.
		if(userFactory.entityList.pagination) {
			$scope.maxSize = 7;
		    $scope.totalItems = userFactory.entityList.pagination.total;
		    $scope.currentPage = userFactory.entityList.pagination.page;
		...
		$scope.getEmployeeList = function () {
			...
			userFactory.getAllData($scope.requestedStatus).then(function(trips) {
	    ...
	    //This method is calling from click on any column
	    $scope.changeSortCol = function (sortCol) {
	        $scope.sortColSelected = sortCol;
	        $scope.getEmployeeList();
	    };
